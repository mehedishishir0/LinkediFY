import Image from "next/image";
import React from "react";
import ProfilePhoto from "./shared/ProfilePhoto";
import { ExternalAccount } from "@/app/page";

type SIdebarProps = {
    user: ExternalAccount;
};

const SIdebar = ({ user }: SIdebarProps) => {


    return (
        <div className="hidden md:block w-[20%] h-fit border border-gray-300 bg-white rounded">
            <div className="flex relative flex-col items-center">
                <div className="w-full h-16 overflow-hidden">
                    {user && (
                        <Image
                            src="/images.jpeg"
                            width={900}
                            height={900}
                            alt="banner"
                            className="w-full h-full rounded-t"
                        />
                    )}
                </div>
                <div className="my-1 absolute top-10 ">
                    <ProfilePhoto src={user && user.imageUrl} />
                </div>
                <div className="border-b border-b-gray-300 ">
                    <div className="p-2 mt-14 text-center ">
                        <h1 className="font-bold hover:underline">{user && user.firstName} {user && user.lastName} </h1>
                        <p className="text-xs">@{user && user.username}</p>
                    </div>
                </div>
            </div>
            <div className="text-xs ">
                <div className="w-full flex gap-3 justify-between items-center px-3 py-2 hover:bg-gray-200 cursor-pointer">
                    <p>Post Impression</p>
                    <p className="text-blue-500 font-bold">88</p>
                </div>
                <div className="w-full flex gap-3 justify-between items-center px-3 py-2 hover:bg-gray-200 cursor-pointer">
                    <p>Posts</p>
                    <p className="text-blue-500 font-bold">0</p>
                </div>
            </div>
        </div>
    );
};

export default SIdebar;
